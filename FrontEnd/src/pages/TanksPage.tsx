...
const TanksPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingTank) {
      ...
      updateMutation.mutate({ id: editingTank.id, data: updateData });
    } else {
      // Create - cần storeId
  ...
  const handleEdit = (tank: Tank) => {
    setEditingTank(tank);
    setSelectedStoreId(tank.storeId);
  ...
  const handleDelete = async (id: number) => {
    const confirmed = await showConfirm(
      'Bạn có chắc chắn muốn xóa bồn bể này?',
      'Xác nhận xóa'
    );
    if (confirmed) {
  ...
  const filteredTanks = tanks?.filter((tank) => {
    const matchesSearch =
      ...
      tank.tankCode.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStore = filterStoreId === 'all' || tank.storeId === filterStoreId;
        ...
        <div className="overflow-x-auto">
              ...
              {filteredTanks?.map((tank) => (
                <tr key={tank.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm font-medium text-gray-900">{tank.tankCode}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm text-gray-900">{tank.name}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm text-gray-500">{getStoreName(tank.storeId)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm text-gray-500">{getProductName(tank.productId)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm text-gray-900">{Number(tank.capacity).toLocaleString()} L</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="text-sm text-gray-900">{Number(tank.currentStock).toLocaleString()} L</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    >
                      {tank.isActive ? 'Hoạt động' : 'Ngưng hoạt động'}
                    </span>
                  </td>
          ...
          {filteredTanks?.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-500">Không tìm thấy bồn bể nào</p>
            </div>
          )}
        </div>
      </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <SearchableSelect
                    ...
                    placeholder="Chọn cửa hàng"
                    isDisabled={!!editingTank}
                    required
                  />
                  <input type="hidden" name="storeId" value={selectedStoreId || ''} required={!editingTank} />
                  {editingTank && (
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Mã bồn <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    name="tankCode"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Tên bồn <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    name="name"
                ...
                <div className="flex items-center">
                  <input
                    ...
                    name="isActive"
                    id="isActive"
                    defaultChecked={editingTank?.isActive ?? true}
