                            ...
                            <div className="space-y-3">
                              ...
                              {ledger.details.receipts && ledger.details.receipts.length > 0 && (
                                <div className="bg-white rounded-lg p-4 border border-yellow-200">
                                  <div className="text-xs font-semibold text-yellow-700 mb-2">🧾 Phiếu thu liên quan</div>
                                  <table className="w-full text-sm">
                                    <thead className="bg-gray-100">
                                      <tr>
                                        <th className="px-3 py-2 text-left">Mã/ID</th>
                                        <th className="px-3 py-2 text-left">Chi tiết</th>
                                        <th className="px-3 py-2 text-right">Số tiền (₫)</th>
                                      </tr>
                                    </thead>
                                    <tbody>
                                      {ledger.details.receipts.map((r: any, idx: number) => (
                                        <tr key={idx} className="border-t">
                                          <td className="px-3 py-2">{r.id}</td>
                                          <td className="px-3 py-2 text-gray-600">{r.notes || (r.customers ? r.customers.map((c: any) => c.customerName).join(', ') : '-')}</td>
                                          <td className="px-3 py-2 text-right tabular-nums font-semibold">{r.amount?.toLocaleString('vi-VN')}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
