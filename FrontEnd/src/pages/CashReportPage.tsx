...
const CashReportPage: React.FC = () => {
  ...
  const handleExportExcel = async () => {
    if (isAllStores) {
      if (!allStoresReport || allStoresReport.length === 0 || !allStoresReport.some(s => s.data?.ledgers?.length > 0)) {
        alert('Không có dữ liệu để xuất');
        return;
      }

      const { workbook, worksheet } = createReportWorkbook('Sổ quỹ tiền mặt - Tất cả cửa hàng');
      ...
      headerRow.values = ['Cửa hàng', 'Số dư đầu kỳ', 'Số phát sinh', 'Số nộp về', 'Số dư cuối kỳ'];
      ...
      allStoresReport.forEach((storeData: any) => {
        if (!storeData.data || !storeData.data.ledgers || storeData.data.ledgers.length === 0) return;
  ...
  const calculateStoreSummary = (ledgers: any[]) => {
    return {
      totalCashIn: ledgers.reduce((sum, ledger) => sum + (ledger.cashIn || 0), 0),
      totalCashOut: ledgers.reduce((sum, ledger) => sum + (ledger.cashOut || 0), 0),
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isAllStores ? !allStoresReport?.some(s => s.data?.ledgers?.length > 0) : !report?.ledgers?.length}
              ...
              <tbody className="bg-white">
                {allStoresReport && allStoresReport.length > 0 ? (
                  allStoresReport.map((storeData: any, storeIndex: number) => {
                    const isStoreCollapsed = collapsedStores.has(storeData.storeId);
                    const storeSummary = calculateStoreSummary(storeData.data?.ledgers || []);
                    return (
                    <React.Fragment key={storeIndex}>
                      ...
                      >
                        <td colSpan={6} className="px-6 py-3 text-left font-bold text-gray-800 border-y">
                      ...
                      {isStoreCollapsed && storeData.data?.ledgers && storeData.data.ledgers.length > 0 && (
                        <>
                          ...
                          <tr className="bg-blue-50 border-b border-blue-200">
                            <td className="px-6 py-3 text-sm font-semibold text-gray-700">Số dư đầu kỳ</td>
                          ...
                          <tr className="bg-amber-50 border-b border-amber-200 hover:bg-amber-100">
                            <td className="px-6 py-3 text-sm font-semibold text-gray-700">Tóm tắt</td>
                            <td className="px-6 py-3"></td>
                            <td className="px-6 py-3 text-right">
                              <div className="text-xs text-gray-600">Phát sinh</div>
                              <div className="text-sm font-bold text-green-600 tabular-nums">
                                +{storeSummary.totalCashIn.toLocaleString('vi-VN')}
                              </div>
                            </td>
                            <td className="px-6 py-3 text-right">
                              <div className="text-xs text-gray-600">Nộp về</div>
                              <div className="text-sm font-bold text-red-600 tabular-nums">
                                -{storeSummary.totalCashOut.toLocaleString('vi-VN')}
                              </div>
                            </td>
                            <td className="px-6 py-3 text-right">
                              <div className="text-xs text-gray-600">Lệch</div>
                              <div className={`text-sm font-bold tabular-nums ${
                                (storeSummary.totalCashIn - storeSummary.totalCashOut) >= 0
                                  ? 'text-green-600'
                                  : 'text-red-600'
                              }`}>
                                {(storeSummary.totalCashIn - storeSummary.totalCashOut).toLocaleString('vi-VN')}
                              </div>
                            </td>
                            <td className="px-6 py-3"></td>
                          </tr>
                          ...
                          <tr className="bg-purple-50 border-b border-purple-200">
                            <td className="px-6 py-3 text-sm font-semibold text-gray-700">Số dư cuối kỳ</td>
                            <td className="px-6 py-3"></td>
                            <td className="px-6 py-3"></td>
                            <td className="px-6 py-3"></td>
                            <td className="px-6 py-3 text-right">
                      ...
                      {showDetails === ledger.id && ledger.details && (
                        <tr>
                          <td colSpan={6} className="px-6 py-4 bg-gray-50">
                            <div className="text-sm font-semibold text-gray-700 mb-2">Chi tiết {getRefTypeLabel(ledger.refType)}:</div>
                            {ledger.details.type === 'RECEIPT' && (
                              <div className="bg-white rounded-lg p-4 border border-green-200 text-xs space-y-2">
                                <div><span className="text-gray-600">Loại phiếu thu:</span> <span className="font-medium">{ledger.details.receiptType === 'CASH_SALES' ? 'Bán lẻ' : 'Thu nợ'}</span></div>
                                <div><span className="text-gray-600">Tổng tiền:</span> <span className="font-bold text-green-600">{ledger.details.totalAmount?.toLocaleString('vi-VN')}</span>₫</div>
                                {ledger.details.receiptAt && <div><span className="text-gray-600">Thời gian thu:</span> <span className="font-medium">{dayjs(ledger.details.receiptAt).format('DD/MM/YYYY HH:mm')}</span></div>}
                                {ledger.details.notes && <div><span className="text-gray-600">Ghi chú:</span> <span className="font-medium">{ledger.details.notes}</span></div>}
                              </div>
                            )}
                            {ledger.details.type === 'DEPOSIT' && (
                              <div className="bg-white rounded-lg p-4 border border-red-200 text-xs space-y-2">
                                <div><span className="text-gray-600">Ngày nộp:</span> <span className="font-medium">{dayjs(ledger.details.depositAt).format('DD/MM/YYYY HH:mm')}</span></div>
                                <div><span className="text-gray-600">Người nhận:</span> <span className="font-medium">{ledger.details.receiverName || '-'}</span></div>
                                <div><span className="text-gray-600">Số tiền:</span> <span className="font-bold text-red-600">{ledger.details.amount?.toLocaleString('vi-VN')}</span>₫</div>
                                {ledger.details.notes && <div><span className="text-gray-600">Ghi chú:</span> <span className="font-medium">{ledger.details.notes}</span></div>}
                              </div>
                            )}
                          </td>
