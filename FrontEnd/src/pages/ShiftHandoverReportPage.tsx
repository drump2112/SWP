...
const ShiftHandoverReportPage: React.FC = () => {
  ...
  const { data: shifts } = useQuery({
    ...
    queryFn: () =>
      selectedStoreId
        ? shiftsApi.getByStore(selectedStoreId)
        : Promise.resolve([]),
    ...
    try {
      ...
      report.pumpReadings.forEach((reading) => {
        ...
        worksheet.getCell(`G${row}`).value =
          (reading.quantity ?? 0) - (reading.testExport ?? 0);
  ...
  const isStoreAccount =
    user?.roleCode === "STORE" ||
    (user?.storeId &&
      !["ADMIN", "DIRECTOR", "ACCOUNTING"].includes(user?.roleCode || ""));
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {report && (
        <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {report.pumpReadings.map((reading, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="border p-2 text-right">
                        {(reading.startValue ?? 0).toFixed(2)}
                      </td>
                      <td className="border p-2 text-right">
                        {(reading.endValue ?? 0).toFixed(2)}
                      </td>
                      <td className="border p-2 text-right">
                        {(reading.quantity ?? 0).toFixed(3)}
                      </td>
                      <td className="border p-2 text-right">
                        {(
                          (reading.quantity ?? 0) - (reading.testExport ?? 0)
                      ...
                      <td className="border p-2 text-right">
                        {(reading.unitPrice ?? 0).toLocaleString("vi-VN")}
                      </td>
                      <td className="border p-2 text-right font-medium">
                        {(reading.amount ?? 0).toLocaleString("vi-VN")}
                      </td>
                    </tr>
                  ))}
                  <tr className="font-bold bg-yellow-50">
                    ...
                    <td className="border p-2 text-right">
                      {report.summary.totalRetailAmount.toLocaleString("vi-VN")}
                    </td>
                  </tr>
                </tbody>
              </table>
                ...
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    ...
                    <tr className="font-bold bg-yellow-50">
                      ...
                      <td className="border p-2 text-right">
                        {report.summary.totalDebtAmount.toLocaleString("vi-VN")}
                      </td>
                      <td className="border p-2"></td>
                    </tr>
                  </tbody>
                ...
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    ...
                    <tr className="font-bold bg-yellow-50">
                      ...
                      <td className="border p-2 text-right">
                        {(
                          report.summary.totalReceiptAmount ?? 0
            ...
            <div className="mb-8">
              ...
              <div className="p-3 bg-red-50 border border-red-200 rounded mb-4">
                <p className="text-sm text-gray-600 font-semibold">
                  Tiền ca trước chuyển sang
                </p>
                <p className="text-lg font-bold text-red-600">
                  {formatCurrency(report.carryOverCash)}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  (Tiền bán lẻ ca trước chưa nộp về công ty)
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-blue-50 rounded">
                <div>
                  ...
                  <p className="text-xl font-bold text-blue-600">
                    {formatCurrency(report.summary.totalRetailAmount)}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-green-600">
                    {formatCurrency(report.summary.totalReceiptAmount)}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-purple-600">
                    {formatCurrency(report.summary.cashBalance)}
                  </p>
                </div>
              </div>
            </div>
          ...
          {report.inventoryByProduct &&
            report.inventoryByProduct.length > 0 && (
              <div className="mb-8">
